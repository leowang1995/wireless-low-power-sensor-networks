...
bool Arpa_RF95::SendMessage(const uint8_t sendToId, const Arpa_msg_type type, const char *data, const uint8_t len)
{
  ...
  if (len > ARPA_MAX_MSG_LENGTH || len < 0)
  {
  ...
  while(true)
  {
    switch(WaitForMessage(buf, &len))
    {
      ...
      default: // If we got a message
        ...
        if(this->fromId == ARPA_BASE_ID) // Message base => node
...
void Arpa_RF95::SetSleepState(const bool state)
{
  // Put device to sleep by simply pulling the enable pin low
  if(state)
  {
    LOG_LN_F("Arpa_RF95::SetSleepState sleeping module");
    digitalWrite(this->en, LOW);
  }
  // Wake up and re-init
  else
  {
    LOG_LN_F("Arpa_RF95::SetSleepState waking and re-initializing module");
    digitalWrite(this->en, HIGH);
...
const bool Arpa_RF95::GetSleepState() const
{
  return this->sleepState;
}

const uint8_t Arpa_RF95::GetFromId() const
{
  return this->fromId;
}

const int16_t Arpa_RF95::GetCurrentConnectionOriginId() const
{
...
const int8_t Arpa_RF95::WaitForSyn()
{
...
void Arpa_RF95::FailureToSendDelay()
{
  delay(random(5000, this->failureDelay));
  this->failureDelay *= 2;
}
